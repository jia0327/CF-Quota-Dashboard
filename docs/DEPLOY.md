...
| # | 步骤 | 说明 |
|---|------|------|
| 1 | 创建 KV 命名空间 | 绑定名必须为 `KV`；根目录 `wrangler.toml` 使用占位符 `YOUR_KV_NAMESPACE_ID` |
...
## 方法零：一键部署（Cloudflare Deploy Button）

README 顶部的 **Deploy to Cloudflare Workers** 按钮会克隆完整仓库并从**根目录**读取 `wrangler.toml`：
...
仓库根目录 `wrangler.toml` 使用占位符，本地部署前需替换：
...
3. **Resolve KV namespace** — 将根目录 `wrangler.toml` 中 `YOUR_KV_NAMESPACE_ID` 替换为真实 ID
4. `npx wrangler deploy --config ../wrangler.toml` 部署 Worker
...
**仓库根目录（Dashboard 一键部署、GitHub Actions、`npm run deploy`）：**
